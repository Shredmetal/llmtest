# Semantic Reliability - Real-World Industry Testing

## Overview

This page documents our extensive testing of llm-app-test against real-world industry use cases. 

We designed this test suite to validate the library's reliability with the kind of content that LLM applications would probably generate in production environments.

The first test of this test suite initially exhibited non-determinism due to a legitimate semantic boundary that the library's author (a lawyer by training) had to read carefully to pick up on. This is documented [here](semantic_boundary_analysis.md). Other than that, the other tests in this suite actually produced the same results in 1,700 runs per test.
...
- Total Runs: 1,200 (600 Windows, 600 Linux)
- Tests per Run: 10
...
Running 12,000 tests using GPT-4o cost approximately US$7.90, demonstrating the economic viability of comprehensive testing even with large test suites.
